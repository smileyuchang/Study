...
public class SysUserServiceImpl extends ServiceImpl<SysUserDao, SysUserEntity> implements SysUserService {
	...
	@Override
	public void importUsers(List<String[]> list) {
		...
		try {
			for (String[] s : list) {
				if (s != null && s[0] != null && s[0] != "" && s[0].indexOf(" ") == -1) {
					...
					//sysUserEntity.setPassword(ShiroUtils.sha256(s[1], sysUserEntity.getSalt()));
					sysUserEntity.setPassword(DigestUtils.sha256Hex(s[1]));
					sysUserEntity.setMobile(s[2]);
